...
<template>
    <Drawer width="500" v-model="isShow" :mask-closable="false" :closable="true">
        <Row class-name="fbold" slot="header">编辑部门</Row>
        <Row>
            <Form ref="editDeptForm" :rules="rules" :model="form" :label-width="80">
                <FormItem label="上级">
            ...
            save(){
                this.$refs.editDeptForm.validate((valid) => {
                    if (valid) {
                        ...
                        this.$http('dept_edit',this.form, true).then(res => {
                            this.loading = false
